...
6. 友人はホーム画面へ追加したアプリの「無料でユーザー登録」からメールアドレスを入力し、メールに届く認証コードで認証します。
...
同じSupabaseプロジェクトを複数人で使う場合も、`app_states.user_id`とRLSにより各利用者のデータは分離されます。認証コードでメールログインできる状態にするには、次の準備が必要です。
...
4. 友人は自分のメールアドレスへ届いた認証コードを、ホーム画面アプリへ省略せず入力します。
